...
Usage: $(basename "${BASH_SOURCE[0]}") [-h] [-v] [-l] repository path
...
msg "${YELLOW}Cloning bare repository to ${args[1]}/$location...${NOFORMAT}"
mkdir ${args[1]}
cd ${args[1]}
...
msg "${YELLOW}Creating default worktrees...${NOFORMAT}"
git worktree add main > /dev/null 2>&1
git worktree add clanker > /dev/null 2>&1
git worktree add review > /dev/null 2>&1
git worktree add pristine > /dev/null 2>&1
